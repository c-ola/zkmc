...
impl JavaUtilRandom {
    ...
    pub fn next_gaussian(&mut self) -> f64 {
        if let Some(next_next_gaussian) = self.next_next_gaussian.take() {
            ...
            loop {
                v1 = 2.0 * self.next_f64() - 1.0;
                v2 = 2.0 * self.next_f64() - 1.0;
                s = v1 * v1 + v2 * v2;
                if !(s >= 1.0 || s == 0.0) {
                    break
                }
    ...
    pub fn ints(&mut self, n: usize) -> Vec<i32> {
        (0..n).into_iter().map(|_| self.next_i32()).collect()
    }
...
impl RandomSource for JavaUtilRandom {
    #[inline]
...
mod rand_tests {
    ...
    pub fn test_next_int() {
        ...
        for i in 0..16 {
            assert_eq!(values[i], random.next_i32());
        }
    }
    ...
    pub fn test_next_long() {
        ...
        for i in 0..16 {
            assert_eq!(values[i], random.next_i64());
        }
    }
    ...
    pub fn test_next_float() {
        ...
        for i in 0..16 {
            assert_eq!(values[i], random.next_f32());
        }
    }
    ...
    pub fn test_next_double() {
        ...
        for i in 0..16 {
            assert_eq!(values[i], random.next_f64());
        }
    }
